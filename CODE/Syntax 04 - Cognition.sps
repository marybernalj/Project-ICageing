...
** Folder: Graph **.
GGRAPH
  /GRAPHDATASET NAME="graphdataset" VARIABLES=year MEAN(cognit)[name="MEAN_cognit"] 
    MISSING=LISTWISE REPORTMISSING=NO
  /GRAPHSPEC SOURCE=INLINE.
BEGIN GPL
  SOURCE: s=userSource(id("graphdataset"))
  DATA: year=col(source(s), name("year"), unit.category())
  DATA: MEAN_cognit=col(source(s), name("MEAN_cognit"))
  GUIDE: axis(dim(1), label("Year"))
  GUIDE: axis(dim(2), label("Mean Cognitive"))
  GUIDE: text.title(label("Simple Line Mean of Cognitive by Year"))
  SCALE: linear(dim(2), include(0))
  ELEMENT: line(position(year*MEAN_cognit), missing.wings())
END GPL.

** Folder: Custom Tables **.
CTABLES
  /VLABELS VARIABLES=cognit year DISPLAY=LABEL
  /TABLE cognit [VALIDN F40.0, MISSING F40.0, TOTALN F40.0, MINIMUM F40.0, PTILE 25 F40.0, MEDIAN 
...
MIXED cognit WITH time quadtime
...
MIXED cognit BY sex site WITH orthtime Zbmi Zphysical Zdrinks Zcigars
  /CRITERIA=DFMETHOD(SATTERTHWAITE) CIN(95) MXITER(100) MXSTEP(10) SCORING(1) 
    SINGULAR(0.000000000001) HCONVERGE(0, ABSOLUTE) LCONVERGE(0, ABSOLUTE) PCONVERGE(0.000001, ABSOLUTE)    
...
** Folder: Graph **.
GGRAPH
  /GRAPHDATASET NAME="graphdataset" VARIABLES=year MEAN(cognit)[name="MEAN_cognit"] sex 
    MISSING=LISTWISE REPORTMISSING=NO
  /GRAPHSPEC SOURCE=INLINE
...
BEGIN GPL
  ...
  GUIDE: text.title(label("Multiple Line Mean of Cognitive by Year by Sex"))
  SCALE: linear(dim(2), include(0))
  SCALE: cat(aesthetic(aesthetic.color.interior), include(
"1", "2"))
  ELEMENT: line(position(year*MEAN_cognit), color.interior(sex), missing.wings())
END GPL.

** Folder: Graph **.
GGRAPH
  /GRAPHDATASET NAME="graphdataset" VARIABLES=year MEAN(cognit)[name="MEAN_cognit"] site 
    MISSING=LISTWISE REPORTMISSING=NO
  /GRAPHSPEC SOURCE=INLINE
  /FRAME OUTER=NO INNER=NO
  /GRIDLINES XAXIS=NO YAXIS=YES
...
BEGIN GPL
  ...
  GUIDE: text.title(label("Multiple Line Mean of Cognitive by Year by Site"))
  SCALE: linear(dim(2), include(0))
  SCALE: cat(aesthetic(aesthetic.color.interior), include(
"1", "5"))
  ELEMENT: line(position(year*MEAN_cognit), color.interior(site), missing.wings())
END GPL.
